...
function launcher.compile_ghc(saveobj)
  local amapper_so = ""
  ...
  local amapper_cc = aroot_dir .. "automated_mapper.cc"
  if os.getenv('OBJNAME') then
    local aout_dir = os.getenv('OBJNAME'):match('.*/') or './'
    amapper_so = aout_dir .. "libautomated_mapper.so"
  elseif saveobj then
    amapper_so = aroot_dir .. "libautomated_mapper.so"
  else
    amapper_so = os.tmpname() .. ".so" -- aroot_dir .. "automated_mapper.so"
  end
  local cxx = os.getenv('CXX') or 'c++'
  local max_dim = os.getenv('MAX_DIM') or '3'
...
-- Compile and link automated_mapper.cc
  ...
  if not saveobj then
    terralib.linklibrary(ghc_binary)
    terralib.linklibrary(mapper_binary)
    mapper_header.register_mappers()
    regentlib.start(toplevel)
  else
    local root_dir = arg[0]:match(".*/") or "./"
    local link_flags = terralib.newlist({"-L" .. root_dir, "-l" .. name, "-lautomated_mapper", "-lm"})
    ...
    regentlib.saveobj(toplevel, "miniaero_GHC", "executable",
