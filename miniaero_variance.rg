...
local launcher_variance = require("launcher_variance")
if os.getenv('SAVE_MAPPER_ONLY') == '1' then
  launcher_variance.compile_mapper(true, "miniaero")
  os.exit(0)
end
...
task output_mapping() --points : region(ispace(int2d), point))
  c.printf("In output mapping\n")
  return --points
end

task stop_timer() --points : region(ispace(int2d), point))
  return --points
end

task start_timer() --points : region(ispace(int2d), point))
  return --points
end

task start_mapping()--points : region(ispace(int2d), point))
  c.printf("Starting mapping\n");
  return --points
end
...
--  end
  ...
  for epoch = 0, 7000 do
    ...
    -- STEP 2: PULL OUT 1 ITER OF SIMULATION
    for time_it = 0, 1 do
    ...
    -- STEP 4: DO REST OF SIMULATION
    for time_it = 1, max_its do
...
launcher_variance.launch(toplevel, "miniaero")
