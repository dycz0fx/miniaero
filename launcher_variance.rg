...
-- Compile and link variance_mapper.cc
do
  local amapper_so = ""
  ...
  local amapper_cc = aroot_dir .. "variance_mapper.cc"
  if os.getenv('OBJNAME') then
    local aout_dir = os.getenv('OBJNAME'):match('.*/') or './'
    amapper_so = aout_dir .. "libvariance_mapper.so"
  elseif os.getenv('SAVEOBJ') == '1' then
    amapper_so = aroot_dir .. "libvariance_mapper.so"
  else
    amapper_so = os.tmpname() .. ".so" -- aroot_dir .. "variance_mapper.so"
  end
  local cxx = os.getenv('CXX') or 'c++'
  local max_dim = os.getenv('MAX_DIM') or '3'
  ...
  if not saveobj then
    ...
    regentlib.saveobj(toplevel, name, "miniaero_executable",
