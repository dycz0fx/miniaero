...
-- Compile and link guided_sample_mapper.cc
function launcher.compile_gsm(saveobj)
  local amapper_so = ""
  ...
  local amapper_cc = aroot_dir .. "guided_sample_mapper.cc"
  if os.getenv('OBJNAME') then
    local aout_dir = os.getenv('OBJNAME'):match('.*/') or './'
    amapper_so = aout_dir .. "libguided_sample_mapper.so"
  elseif saveobj then
    amapper_so = aroot_dir .. "libguided_sample_mapper.so"
  else
    amapper_so = os.tmpname() .. ".so" -- aroot_dir .. "guided_sample_mapper.so"
  end
  local cxx = os.getenv('CXX') or 'c++'
  local max_dim = os.getenv('MAX_DIM') or '3'
...
function launcher.launch(toplevel, name)
  ...
  local gsm_binary = launcher.compile_gsm(saveobj)
  ...
  if not saveobj then
    terralib.linklibrary(gsm_binary)
    terralib.linklibrary(mapper_binary)
    mapper_header.register_mappers()
    regentlib.start(toplevel)
  else
    local root_dir = arg[0]:match(".*/") or "./"
    local link_flags = terralib.newlist({"-L" .. root_dir, "-lminiaero_guided_sample", "-lguided_sample_mapper", "-lm"})
    ...
    regentlib.saveobj(toplevel, "miniaero_GSM", "executable",
