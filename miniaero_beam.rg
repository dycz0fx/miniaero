...
local AUTOMAP = require("automap")
local cmath = terralib.includecstring [[
#include <math.h>
#define DBL_EPSILON 2.2204460492503131e-16
]]

local launcher_beam = require("launcher_beam")
if os.getenv('SAVE_MAPPER_ONLY') == '1' then
  launcher_beam.compile_mapper(true, "miniaero")
  os.exit(0)
end
...
task rewrite_blocks(blocks : region(Block),
                    rcell : region(Cell),
                    rface : region(Face(rcell, rcell)))
where reads writes(blocks, rcell, rface)
do
  c.printf("Pretended to rewrite blocks\n")
  return 1
end
...
task toplevel()
  ...
  for epoch = 0, 100000 do
    ...
    for time_it = 0, max_its do
      -- After the first iteration, let AutoMap begin profiling.
      if time_it == 1 then
        AUTOMAP.begin_profile()
      end
    ...
    -- After the epoch ends, end the profiling phase.
    AUTOMAP.end_profile()
  end
...
launcher_beam.launch(toplevel, "miniaero")
