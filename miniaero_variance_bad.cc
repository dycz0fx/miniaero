...
class MiniAeroMapper : public VarianceBadMapper
{
public:
...
MiniAeroMapper::MiniAeroMapper(MapperRuntime *rt, Machine machine, Processor local,
                               const char *mapper_name)
  : VarianceBadMapper(rt, machine, local, mapper_name)
{
  //add_task_to_care(...)
...
//  add_task_to_care("Vec3Average");
  ...
  name_application("miniaero_variance_bad");
}
