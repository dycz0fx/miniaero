...
-- Compile and link hardware_simulator.cc
do
  ...
  local amapper_cc = aroot_dir .. "hardware_simulator.cc"
  if os.getenv('OBJNAME') then
    local aout_dir = os.getenv('OBJNAME'):match('.*/') or './'
    amapper_so = aout_dir .. "libhardware_simulator.so"
  elseif os.getenv('SAVEOBJ') == '1' then
    amapper_so = aroot_dir .. "libhardware_simulator.so"
  else
    amapper_so = os.tmpname() .. ".so"
  end
  local cxx = os.getenv('CXX') or 'c++'
  local max_dim = os.getenv('MAX_DIM') or '3'
  ...
  terralib.linklibrary(amapper_so)
  -- amapper = terralib.includec("specified_mapper.h", {"-I", aroot_dir, "-I", aruntime_dir})
end
print("Linked hardware_simulator.cc");


-- Compile and link specified_mapper.cc
function launcher.compile_specified(saveobj)
  local aroot_dir = arg[0]:match(".*/") or "./"
  local amapper_so = aroot_dir .. "libspecified_mapper.so"
  return amapper_so
end
...
function launcher.launch(toplevel, name)
  ...
  local specified_binary = launcher.compile_specified(saveobj)
  ...
  if not saveobj then
    terralib.linklibrary(specified_binary)
    ...
    print("name: " .. name)
    local root_dir = arg[0]:match(".*/") or "./"
    local link_flags = terralib.newlist({"-L" .. root_dir, "-lminiaero_specified", "-lspecified_mapper", "-lm"})
    if USE_LPMIX then
    ...
    regentlib.saveobj(toplevel, "miniaero_specified", "executable",
